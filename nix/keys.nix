...

{
  ...
  config = {
    ...
    assertions = (flip mapAttrsToList config.deployment.keys (key: opts: {
      assertion = (opts.text == null && opts.keyFile != null && opts.keyCmd == null) ||
                  (opts.text != null && opts.keyFile == null && opts.keyCmd == null) ||
                  (opts.text == null && opts.keyFile == null && opts.keyCmd != null);
      message = "Deployment key '${key}' must have either a 'text', 'keyCmd' or a 'keyFile' specified.";
    })) ++ (flip mapAttrsToList config.deployment.keys (key: opts: {
      # storeKeysOnMachine -> (keyCmd == null) == !storeKeysOnMachine || (keyCmd == null)
      assertion = (!config.deployment.storeKeysOnMachine || opts.keyCmd == null);
      messages = "Deployment key '${key}' specifies 'keyCmd' but deployment.storeKeysOnMachine is 'true', which is not compatible with each other.";
    }));
